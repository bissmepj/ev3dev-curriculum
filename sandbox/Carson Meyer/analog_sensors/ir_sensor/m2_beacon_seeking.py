...
def seek_beacon(robot):
    ...
    while not robot.touch_sensor.is_pressed:
        ...
        if current_distance == -128:
            ...
            if math.fabs(current_heading) <= 30:
                robot.forward(forward_speed, forward_speed)

            if math.fabs(current_heading) > 2 and \
                math.fabs(current_heading) < 10:
    ...
    print("Abandon ship!")
    ev3.Sound.speak("Mission Failed We'll get them next time")
    robot.stop()
    return False
