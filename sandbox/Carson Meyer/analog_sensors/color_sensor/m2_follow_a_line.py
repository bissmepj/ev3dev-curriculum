...
def main():
    ...
    print(" Follow a line")
    print("--------------------------------------------")
    ev3.Sound.speak("I walk the line").wait()
...
def follow_the_line(robot, white_level, black_level):
    ...
    while True:
        if robot.color_sensor.reflected_light_intensity >= 80:
            robot.forward(350, 350)

        if robot.color_sensor.reflected_light_intensity <= 25:
            robot.turn_degrees(5, 350)
            count = count + 1

        if count == 25:
            robot.stop()
            ev3.Sound.speak("I am dizzy")
