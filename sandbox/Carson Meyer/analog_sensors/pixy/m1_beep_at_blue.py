...
def main():
    ...
    pixy = ev3.Sensor(driver_name="pixy-lego")            #Gives the pixy sensor a value name
    assert pixy                                           #Checks to see if the sensor is actually attatched
    robot = robo.Snatch3r()
    ...
    while not robot.touch_sensor.is_pressed:
        ...
        width = pixy.value(3)                       #These values are used to gather and store data from the pixy camera
        height = pixy.value(4)
        x = pixy.value(1)
        y = pixy.value(2)
