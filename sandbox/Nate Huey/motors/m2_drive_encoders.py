...
def main():
    ...
    while time_s != 0:
        ...
        if dist ==0:
            break
        degrees_per_inch = 90
        motor_turns_needed = degrees_per_inch * dist
        left_motor.run_to_rel_pos(speed_sp=speed, position_sp=motor_turns_needed, stop_action=ev3.Motor.STOP_ACTION_BRAKE )
        right_motor.run_to_rel_pos(speed_sp=speed, position_sp= motor_turns_needed, stop_action=ev3.Motor.STOP_ACTION_BRAKE)
