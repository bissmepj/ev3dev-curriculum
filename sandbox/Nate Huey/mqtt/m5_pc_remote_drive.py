...
def main():
    ...
    left_button['command'] = lambda: some_callback1(mqtt_client, left_speed_entry, right_speed_entry)
    root.bind('<Left>', lambda event: some_callback1(mqtt_client, left_speed_entry, right_speed_entry))

    stop_button = ttk.Button(main_frame, text="Stop")
    stop_button.grid(row=3, column=1)
    # stop_button and '<space>' key (note, does not need left_speed_entry, right_speed_entry)
    stop_button['command'] = lambda: some_callback1(mqtt_client)
    root.bind('<space>', lambda event: some_callback1(mqtt_client ))

    right_button = ttk.Button(main_frame, text="Right")
    right_button.grid(row=3, column=2)
    # right_button and '<Right>' key
    right_button['command'] = lambda: some_callback1(mqtt_client, left_speed_entry, right_speed_entry)
    root.bind('<Right>', lambda event: some_callback1(mqtt_client, left_speed_entry, right_speed_entry))
